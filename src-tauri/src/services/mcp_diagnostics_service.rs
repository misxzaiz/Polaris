...
impl TodoMcpDiagnosticsService {
    pub fn collect(
        ...
        let workspace_config_exists = workspace_config_path.as_ref().map(|path| path.exists());
        ...
        Ok(TodoMcpDiagnostics {
            ...
            expected_resource_bin_path: resource_dir.as_ref().map(|dir| {
                dir.join(format!("bin/polaris-todo-mcp{}", EXE_SUFFIX))
                    .to_string_lossy()
                    .to_string()
            }),
            expected_resource_root_path: resource_dir.as_ref().map(|dir| {
                dir.join(format!("polaris-todo-mcp{}", EXE_SUFFIX))
                    .to_string_lossy()
                    .to_string()
            }),
