//! cap.data_root —— 数据根管理统一入口（第七步阶段 C：管理面域上总线）
//!
//! 对应 `dev/docs/sky/step7-consolidation.md` 阶段 C：
//! - 抽核自旧命令层 `commands/data_root_cmd.rs`（业务核 + 类型逐行搬移，语义不变）
//! - 走 RouterBus dispatch（cap.data_root），获得统一权限 gate + 审计 + Source 注入
//! - `open_path_in_explorer` 属平台壳命令（step7 §3 边界：文件对话框/资源管理器
//!   不进总线），保留在命令层
//!
//! # 动作协议（payload `{ "action": ... }`）
//!
//! - `get_info`     `{ "action": "get_info" }`
//!                            → DataRootInfo（当前数据根路径/占用/子目录）
//! - `scan_legacy`  `{ "action": "scan_legacy" }`
//!                            → Vec<LegacySource>（旧版残留数据源）
//! - `migrate`      `{ "action": "migrate", "options": { "sources": [...], "overwrite": bool } }`
//!                            → MigrateReport（合并旧数据到当前数据根）
//! - `validate_target` `{ "action": "validate_target", "options": { "newPath": string|null,
//!                            "mode": "switch_only"|"move_data" } }`
//!                            → TargetValidation（切换目标路径校验）
//! - `set_root`     `{ "action": "set_root", "options": { "newPath": string|null,
//!                            "mode": "switch_only"|"move_data" } }`
//!                            → SetDataRootReport（切换锚点，需重启生效）
...
// ============================================================================
// 业务核（抽核自 data_root_cmd.rs，语义逐行保持）
// ============================================================================
...
/// cap.data_root —— 数据根管理能力（无状态，纯函数业务核）
pub struct DataRootCapability;

const CAP_ID: &str = "cap.data_root";

/// 支持的 action 协议（写死为契约，与前端对齐）
pub const DATA_ROOT_ACTIONS: &[&str] = &[
    "get_info",
    "scan_legacy",
    "migrate",
    "validate_target",
...
fn parse_options<T: serde::de::DeserializeOwned>(
    ...
    .map_err(|e| format!("options 参数解析失败: {}", e))
}

impl Capability for DataRootCapability {
    ...
    fn invoke(&self, params: Value, _ctx: &dyn Context) -> std::result::Result<Value, String> {
        let action = params
            .get("action")
            .and_then(|a| a.as_str())
            .ok_or_else(|| format!("cap.data_root 需要 action 参数（{:?}）", DATA_ROOT_ACTIONS))?;

        match action {
            "get_info" => {
                let info = data_root_info_inner().map_err(|e| e.to_string())?;
                serde_json::to_value(info).map_err(|e| e.to_string())
            }
            "scan_legacy" => {
                let list = scan_legacy_data_inner().map_err(|e| e.to_string())?;
                serde_json::to_value(list).map_err(|e| e.to_string())
            }
            "migrate" => {
                let options = parse_options::<MigrateOptions>(&params)?;
                let report = migrate_legacy_data_inner(options).map_err(|e| e.to_string())?;
                serde_json::to_value(report).map_err(|e| e.to_string())
            }
            "validate_target" => {
                let options = parse_options::<SetDataRootOptions>(&params)?;
                let result = validate_target_inner(&options).map_err(|e| e.to_string())?;
                serde_json::to_value(result).map_err(|e| e.to_string())
            }
            "set_root" => {
                let options = parse_options::<SetDataRootOptions>(&params)?;
                let report = set_data_root_inner(options).map_err(|e| e.to_string())?;
                serde_json::to_value(report).map_err(|e| e.to_string())
            }
            other => Err(format!("cap.data_root 不支持动作: {}", other)),
        }
    }
