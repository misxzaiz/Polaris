...
/// 输出预留（token）
const RESERVED_OUTPUT_TOKENS: u64 = 2000;
...
pub(super) const MANUAL_MIN_TAIL_TURNS: usize = 1;
...
impl TokenEstimator {
    ...
    pub fn estimate_request_size(
        ...
        // messages 已包含 system/pinned/current-user，必须只计一次。
        let messages_size: usize = messages.iter().map(estimate_message_size).sum();
        let tools_size: usize = tools.iter().map(estimate_tool_spec_size).sum();

        // 协议转换开销（角色包装、content block、tool schema 等）。
        let protocol_overhead = match wire_protocol {
            WireProtocol::OpenAIChat => 1.05,
            WireProtocol::Anthropic => 1.20,
            WireProtocol::Responses => 1.15,
        };
        let raw_estimate = messages_size.saturating_add(tools_size);
        ((raw_estimate as f64) * self.calibration_factor() * protocol_overhead).ceil() as usize
    }
...
fn estimate_message_size(msg: &Value) -> usize {
    ...
    if let Some(role) = msg.get("role") {
        size += role.as_str().map_or(0, |s| s.len());
    }

    if let Some(content) = msg.get("content") {
        size += content.as_str().map_or(0, |s| s.len());
    }

    if let Some(tool_calls) = msg.get("tool_calls").and_then(|tc| tc.as_array()) {
        for tool_call in tool_calls {
            size += serde_json::to_string(tool_call).map_or(0, |s| s.len());
        }
    }

    if let Some(tool_call_id) = msg.get("tool_call_id") {
        size += tool_call_id.as_str().map_or(0, |s| s.len());
    }
...
/// 估算工具定义的 token 大小
fn estimate_tool_spec_size(spec: &Value) -> usize {
...
pub fn build_compaction_plan(
    ...
    let estimated_size = estimator.estimate_request_size(messages, tools, context_window, wire_protocol);
    ...
    // 强制保证尾部之前至少有一个可压缩回合（否则压缩无意义）。
    // tail_turn_start 必须 ≤ turns_with_tokens.len() - 1。
    let tail_turn_start = tail_turn_start.min(turns_with_tokens.len().saturating_sub(1));
    ...
    let estimated_after = estimated_size
        .saturating_sub(compactable_turns.iter().map(|t| t.estimated_tokens).sum::<usize>());
            ...
            "assistant" => {
                ...
                if let Some(tc_array) = msg.get("tool_calls").and_then(|tc| tc.as_array()) {
                    for tc in tc_array {
                        ...
                        parts.push(format!("[assistant tool_call]: {}(id={}, args={})", name, call_id, args));
                    }
                }
                if parts.is_empty() {
...
mod tests {
    ...
    fn build_compaction_plan_with_exactly_min_turns() {
        ...
        // 恰好 3 回合，tail 保留 1 个 → 2 个可压缩
        assert!(!plan.compactable_turns.is_empty());
        assert_eq!(plan.recent_tail_turn_count, 1);
    }
