 ...
 *
 * 扫描 `<DataRoot>/agents/<name>.md`：YAML frontmatter（`name` / `description`
 * / `tools`）+ body（system prompt）。`options.agent` 指定时，body 覆盖默认 persona
 * （用户显式传 `system_prompt` 时仍完全覆盖，决策 §12-3）。
 *
 * 专家全局存储(2026-07 重构):专家落 `<DataRoot>/agents/<slug>.md`,与专家团
 * `rosters-user.json` 同目录,与其他设置统一。由 AI 经 MCP `save_agent` 自助维护,
 * 或用户手工放置。`load_agent_def` 供 claude 引擎与派发链路读取专家 body 注入人格。
 */
...
pub(crate) struct AgentDefinition {
    /// 文件名 stem(= slug,`load_agent`/`options.agent` 的检索键)
    pub slug: String,
    pub name: String,
...
/// agent 查找目录:`<DataRoot>/agents/`(全局)。
fn agent_dirs() -> Vec<std::path::PathBuf> {
    vec![crate::services::data_root::data_root().root().join("agents")]
}

/// 扫描所有 agent 定义（供前端选择器）。全局。
...
/// 读取全局专家定义的 `(slug, description, system_prompt)`,供 claude 引擎
/// `--agents <json>` 免落盘注入与派发链路人格注入。
///
/// 解析 `<DataRoot>/agents/<slug>.md` 的 frontmatter `description` 与 body。
/// 命中失败返回 None(调用方回退默认行为)。
pub fn load_agent_def(slug: &str) -> Option<(String, String, String)> {
    let dir = crate::services::data_root::data_root().root().join("agents");
    load_agent_def_in(&dir, slug)
}

/// 从指定目录读取专家定义(可测变体,供 `load_agent_def` 与测试使用)。
fn load_agent_def_in(dir: &Path, slug: &str) -> Option<(String, String, String)> {
...
mod tests {
    ...
    fn load_agent_by_name() {
        ...
        .unwrap();
        let agent = load_agent_in(&[agents_dir.clone()], "reviewer").unwrap();
        assert_eq!(agent.name, "reviewer");
        assert!(agent.system_prompt.contains("code reviewer"));
        // 缺失返回 None
        assert!(load_agent_in(&[agents_dir], "nope").is_none());
    }
    ...
    fn project_agents_shadow_global_corpus() {
        // discover_agents_in 接受显式目录列表,前面目录优先(去重语义)。
        // 生产路径只传全局 agents 一个目录;此测试验证多目录去重逻辑。
        let dir = tempfile::tempdir().unwrap();
        let primary = dir.path().join("primary");
        let secondary = dir.path().join("secondary");
        fs::create_dir_all(&primary).unwrap();
        fs::create_dir_all(&secondary).unwrap();
        fs::write(primary.join("dup.md"), "---\nname: primary-dup\n---\np").unwrap();
        fs::write(secondary.join("dup.md"), "---\nname: secondary-dup\n---\ns").unwrap();
        fs::write(secondary.join("only-secondary.md"), "---\nname: only-secondary\n---\ns").unwrap();

        let dirs = vec![primary, secondary];
        let agents = discover_agents_in(&dirs);
        assert_eq!(agents.len(), 2);
        assert!(agents.iter().any(|a| a.name == "primary-dup"));
        assert!(agents.iter().any(|a| a.name == "only-secondary"));
        assert!(!agents.iter().any(|a| a.name == "secondary-dup"));

        // load 同样前面目录优先
        assert_eq!(load_agent_in(&dirs, "dup").unwrap().name, "primary-dup");
        assert_eq!(load_agent_in(&dirs, "only-secondary").unwrap().name, "only-secondary");
        assert!(load_agent_in(&dirs, "nope").is_none());
    }

    #[test]
    fn load_agent_def_reads_global() {
        let dir = tempfile::tempdir().unwrap();
        let agents_dir = dir.path().join("agents");
        fs::create_dir_all(&agents_dir).unwrap();
        fs::write(
        ...
        .unwrap();
        let (slug, desc, body) = load_agent_def_in(&agents_dir, "reviewer").unwrap();
        assert_eq!(slug, "reviewer");
        assert_eq!(desc, "code review");
        assert!(body.contains("代码审查专家"));
        // 缺失返回 None
        assert!(load_agent_def_in(&agents_dir, "nope").is_none());
    }
}
