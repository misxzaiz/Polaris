...
pub fn get_usage_summary(
    ...
    db.get_summary(start_date, end_date, model.as_deref())
}
...
pub fn get_usage_model_stats(
    ...
    let db = usage_db::get_usage_db()
        .ok_or_else(|| AppError::StateError("用量数据库未初始化".to_string()))?;
    db.get_model_stats(start_date, end_date)
}
...
pub fn get_usage_daily_trends(
    ...
    let db = usage_db::get_usage_db()
        .ok_or_else(|| AppError::StateError("用量数据库未初始化".to_string()))?;
    db.get_daily_trends(start_date, end_date, model.as_deref())
}
...
pub fn get_usage_recent_logs(
    ...
    let db = usage_db::get_usage_db()
        .ok_or_else(|| AppError::StateError("用量数据库未初始化".to_string()))?;
    db.get_recent_logs(limit.unwrap_or(20), start_date, end_date, model.as_deref())
}