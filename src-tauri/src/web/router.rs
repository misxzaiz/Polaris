...
fn resolve_dist_dir(state: &AppState) -> std::path::PathBuf {
    ...
    if let Some(Some(resource_dir)) = state.resource_dir.get().as_ref() {
        ...
        if index_html.exists() {
            tracing::info!(
                "[Web] SPA dist resolved via resource_dir (flat): {:?}",
                resource_dir
            );
        ...
        tracing::debug!(
            "[Web] resource_dir not available (None or unset), trying exe-relative paths"
        );
    }
    ...
    if let Ok(exe_path) = std::env::current_exe() {
        if let Some(exe_dir) = exe_path.parent() {
            ...
            if let Some(parent) = exe_dir.parent() {
                ...
                if candidate.is_dir() {
                    tracing::info!(
                        "[Web] SPA dist resolved via exe-parent-relative: {:?}",
                        candidate
                    );
                    return candidate;
                }
...
pub fn create_router(state: Arc<AppState>) -> Router {
    ...
    let api_routes = Router::new()
        ...
        .route("/chat/compact", post(api::chat::handle_compact_context))
        .route(
            "/chat/restore-compacted-context",
            post(api::chat::handle_restore_compacted_context),
        )
        .route(
            "/chat/checkpoints/delete",
            post(api::chat::handle_delete_context_checkpoints),
        )
        .route(
            "/chat/history/{session_id}",
            get(api::chat::handle_get_history),
        )
        .route(
            "/chat/answer-question",
            post(api::chat::handle_answer_question),
        )
        .route(
            "/chat/respond-plugin-card",
            post(api::chat::handle_respond_plugin_card),
        )
        .route("/chat/approve-plan", post(api::chat::handle_approve_plan))
        .route("/chat/reject-plan", post(api::chat::handle_reject_plan))
        // Artifacts
        .route(
            "/artifacts/codex-images/{thread_id}/{file_name}",
            get(api::artifacts::handle_codex_image_artifact),
        )
        // Sessions
        .route(
            "/sessions",
            get(api::session::handle_list_sessions).post(api::session::handle_create_session),
        )
        .route(
            "/sessions/{id}",
            delete(api::session::handle_delete_session),
        )
        // Legacy Claude Code session endpoints (return flat arrays, not PagedResult)
        .route(
            "/claude-sessions",
            get(api::session::handle_list_claude_sessions),
        )
        .route(
            "/claude-sessions/{session_id}/history",
            get(api::session::handle_get_claude_session_history),
        )
        // Settings
        .route(
            "/settings",
            get(api::settings::handle_get_settings).patch(api::settings::handle_update_settings),
        )
        // Auth
