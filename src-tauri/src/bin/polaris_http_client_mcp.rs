//! HTTP Client MCP Binary Entry Point
//!
//! Usage: polaris-http-client-mcp <config_dir> [workspace_path]

use polaris_lib::services::http_client_mcp_server::run_http_client_mcp_server;
...
fn main_impl() -> Result<()> {
    ...
    if args.len() < 2 {
        return Err(AppError::ValidationError(
            "缺少参数。用法：polaris-http-client-mcp <config_dir> [workspace_path]".to_string(),
        ));
    }

    let (config_dir, workspace_path) = parse_args(&args)?;

    run_http_client_mcp_server(&config_dir, workspace_path)
}
...
fn parse_args(args: &[String]) -> Result<(String, Option<&str>)> {
    match args.len() {
        ...
        _ => Err(AppError::ValidationError(
            "参数过多。用法：polaris-http-client-mcp <config_dir> [workspace_path]".to_string(),
        )),
    }
}
