...
采用 **同一可视对话、同一正常运行时会话内的混合式上下文交接压缩**。压缩后的模型上下文由三部分组成：

```text
Pinned Bootstrap Context
+ Structured Handoff Briefing
+ Token-Budgeted Recent Exact Turns
```

被移出的旧历史写入 Polaris 应用数据目录中的本地恢复档案，不继续出现在每次模型请求中。后续如需让模型查询旧细节，通过专用、受限的上下文档案读取工具完成，而不是直接暴露应用数据绝对路径或让通用文件工具整份读入。

自动和手动压缩必须调用同一个 `CompactionCoordinator`，共享相同的分组、预算、快照、摘要、验证、提交和错误恢复逻辑。
...
写入必须原子化：先写临时文件并校验，再 rename 为 generation 文件，最后更新 manifest。只有恢复档案写入成功、briefing 有效且重建后的请求低于目标预算，才允许替换 `session.messages`。
...
### D10. 正常压缩不旋转 runtime session

正常自动/手动压缩保持现有 runtime session ID，避免打断：

- 事件路由；
- 中断状态；
- MCP/工具会话状态；
- frontend conversationId 与 JSONL externalId；
- 用户对“仍在同一对话”的认知。

创建新 runtime 只用于：

- 应用或后端重启后原内存 session 丢失；
- session 被回收；
- 用户显式选择“在新上下文继续”；
- provider/wire protocol 不兼容切换；
- 运行时状态损坏。

长期必须拆分：

```text
stableConversationId  // UI 与持久化身份
runtimeSessionId      // 当前引擎进程身份
```

短期先删除 `continue_session` 中“找不到 session 就只带 system 静默继续”的行为，改为类型化 `SessionNotFound`，由前端明确提示恢复。恢复流程使用最新 checkpoint 的 pinned context + briefing + recent exact tail 创建新 runtime，同时保持原可视对话页。

## Considered Alternatives

### Alt 1：继续使用 summary-only 原地替换

优点是改动最小。缺点是精确工具证据、最近措辞和纠错过程会丢失，多次摘要产生累积失真，且没有可靠恢复。当前实现已经证明失败路径容易退化为删 turn。**否决作为主方案**。

### Alt 2：每次压缩都创建新的 SimpleAI runtime session

能够得到干净上下文，但当前 runtime ID 同时参与 frontend conversationId、JSONL externalId 和事件路由。常规旋转会造成会话身份分裂、工具状态重建和用户感知中断。**否决作为常规方案，仅用于显式恢复**。

### Alt 3：优先采用供应商原生 compaction/context management
...
| 重复压缩摘要漂移 | briefing 再被反复摘要 | 每次新 checkpoint 合并原始归档与新回合，briefing 输入基于结构化历史状态；保留 generation |

## Implementation Plan

### Phase 0：正确性前置修复

1. 同一 SimpleAI session 一次只允许一个 active turn；拒绝或排队重叠 continue。
2. 用 `turn_generation` 校验回写，防止旧任务覆盖新历史。
3. 删除 `continue_session` 的 system-only 静默恢复，返回类型化 `SessionNotFound`。
4. 移除自动 `fallback_drop_oldest`。
5. 将常规压缩移出工具循环中间，只允许 safe boundary。
6. 统一 `SessionEnd` 事件所有权，避免中断路径重复发送。
...
### Phase 2：统一 CompactionCoordinator 与手动入口

1. 用 `CompactionCoordinator::compact(trigger)` 替换现有 `compact_history`。
2. 流程固定为：plan → checkpoint → summarize → validate → atomic commit。
3. 生成结构化 user-role handoff briefing。
4. 保留 token-budgeted recent exact turns。
5. 新增结构化 `ContextCompactionEvent`，不要只依赖任意文本 `ProgressEvent`。
...
1. 分离 `stableConversationId` 和 `runtimeSessionId`。
2. 运行时丢失时从最新 checkpoint 恢复 pinned context + briefing + recent tail。
3. 前端保持同一 tab 和历史身份，显示“已从本地对话状态恢复运行时”。
4. 将 checkpoint 清理接入对话删除和 session LRU/TTL 回收。
...
- DataRoot / dialog lifecycle 相关存储模块
