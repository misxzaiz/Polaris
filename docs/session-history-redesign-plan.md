...
> 状态：**Phase 0–3 已实施**（2026-07-18）。
> 已落地：合并保护/增量 append/前缀保留、Web 统一后端存储 + OPFS 自动迁移、
> SQLite 索引（FTS5 trigram 全文搜索）+ history_query/search/mark、统一时间线面板、
> 尾部优先分页恢复 + 「加载更早」、继续工作区 + 星标/置顶/归档 + 体验配置。
> 未实施（后续迭代）：保存状态指示器（0.4）、AI 摘要标题（3.4 可选）、Phase 4 生命周期打磨。
