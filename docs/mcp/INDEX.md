...
当前已落地一版宿主编排 + MCP 工具面的长期目标闭环：长期目标文档服务、Tauri/Web IPC 命令、前端 typed wrapper、长期目标左侧面板、规划/执行/维护会话 prompt 生成、从面板创建新的 AI 会话并发送 prompt、目标创建后可自动启动第一次规划会话、把运行中的 `sessionId` 写回 `goal.json`、长期目标面板中断当前运行会话、监听 `session_end` 后兜底写回会话摘要并清空 `currentSessionId`、按 `nextRunAt` 自动开启下一次执行会话、自动失败时按重试退避重新排期并在超过上限后标记 `blocked`、创建目标时配置代码修改和 git 提交权限、completed 状态下的完成复审入口，以及目标详情运行状态可视化。另已新增 `examples/plugins/long-goal-mcp-plugin` 外部 MCP 插件样例骨架，以及正式 Rust `polaris-long-goal-mcp` / `polaris.long-goal` MCP surface。`polaris.long-goal` 现在同时贡献受控宿主可视化入口和 `polaris-long-goal` MCP server；AI 长期目标会话会优先通过 MCP tools 读取、更新文档和记录进度，宿主 session tracker 作为兜底写回。
...
长期目标 MCP 本阶段已完成“受控 UI 入口 + Rust MCP 工具面 + 会话 prompt MCP 优先 + 兜底写回 + 状态机约束”的闭环。后续优先进入插件系统通用能力：外部前端代码动态加载与隔离、插件权限执行边界、远程来源/市场和外部 MCP server 分发权限审计。

建议下一次任务入口：

> 读取 `docs/mcp/INDEX.md`、`docs/mcp/2-roadmap.md`、`docs/mcp/4-manifest-schema.md` 和插件设置/安装相关代码，继续推进插件系统阶段 7：权限执行边界、外部前端代码隔离策略和远程来源管理。
