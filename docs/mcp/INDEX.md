...
优先推进长期目标会话编排：下一步建议把执行 prompt 和会话结束写回逻辑调整为优先使用 `polaris-long-goal` MCP tools，并继续收敛 `LongGoalService` 的状态机约束。外部 MCP 插件化迁移方案已补充到长期目标方案文档。

建议下一次任务入口：

> 读取 `docs/mcp/INDEX.md`、`docs/mcp/6-long-goal-executor-plugin.md`、`src-tauri/src/services/long_goal_mcp_server.rs`、`src-tauri/src/services/long_goal_service.rs`、`src/services/longGoalSessionTracker.ts`，继续迁移长期目标执行写回链路，让 AI 会话优先通过 `polaris-long-goal` MCP tools 读取和记录长期目标进度。
