...
优先推进长期目标会话编排：下一步建议验证 `examples/plugins/long-goal-mcp-plugin` 的安装发现流程，并开始把宿主侧长期目标文档服务抽成内置 Tauri 命令和外部 MCP server 可共享的稳定边界。外部 MCP 插件化迁移方案已补充到长期目标方案文档。

建议下一次任务入口：

> 读取 `docs/mcp/INDEX.md`、`docs/mcp/6-long-goal-executor-plugin.md`、`docs/mcp/4-manifest-schema.md`、`examples/plugins/long-goal-mcp-plugin` 和现有插件安装测试，验证长期目标外部 MCP 插件样例的安装发现流程，并开始抽取长期目标文档服务的内外共用边界。
