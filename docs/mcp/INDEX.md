...
优先推进长期目标会话编排：下一步建议验证 `examples/plugins/long-goal-mcp-plugin` 的安装发现流程，并决定是否把宿主侧长期目标文档服务抽成可复用库，供内置 Tauri 命令和外部 MCP server 共用。外部 MCP 插件化仍保留为后续阶段，当前先验证宿主侧状态机、权限和 UX。

建议下一次任务入口：

> 读取 `docs/mcp/INDEX.md`、`docs/mcp/6-long-goal-executor-plugin.md`、`docs/mcp/4-manifest-schema.md`、`examples/plugins/long-goal-mcp-plugin` 和现有插件安装测试，验证长期目标外部 MCP 插件样例的安装发现流程，并评估是否把长期目标文档服务抽成内外共用库。
