...
优先继续补齐“插件安装与发现流程”的可靠性与来源管理：在已有目录扫描、已安装插件清单返回、设置页刷新、manifest contribution 诊断、本地目录安装和安全卸载基础上，增加 manifest schema 校验命令、安装包/远程来源安装、插件更新检查与更明确的安装失败诊断。

建议下一次任务入口：

> 读取 `docs/mcp/INDEX.md`、`docs/mcp/2-roadmap.md` 和 `docs/mcp/4-manifest-schema.md`，继续阶段 6：插件 manifest schema 校验命令、安装包/远程来源安装、插件更新检查与安装失败诊断。
