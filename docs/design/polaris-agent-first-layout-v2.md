...
- 侧栏高密度面板：`FileExplorer`、`GitPanel`、`KnowledgePanel`、`TerminalPanel`、`SessionHistoryPanel`
...
左侧 ActivityBar 目前是功能列表：文件、Git、Todo、翻译、Scheduler、Requirement、Terminal、Developer、Integration、Knowledge、Problems。功能很全，但和当前 Agent session 的关系没有被表达出来。
...
- Knowledge、Requirement、Todo 作为 Agent 上下文来源，不只是独立工具面板。
...
- Context：Files、Knowledge、Requirements；
- Changes：Git、Problems；
- Run：Terminal、Scheduler、Long Goal；
...
- 左侧上下文侧栏变成操作台；
- 文件、Git、Knowledge、Terminal 都在同一区域用分段切换；
...
- 统一 FileExplorer、Git、Knowledge、Terminal 的面板头部；
...
- 切换 Context Sidebar 的 Files / Git / Knowledge / Terminal；
