 ...
 *
 * 钉钉机器人接入：
 * - 接收消息：WebSocket 长连接（open-dingtalk.com，需在开放平台启用「Stream 模式」）
 * - 发送回复：企业机器人 Webhook URL（群机器人 Webhook）
 */
...
const log = createLogger('DingTalkTab');
...
function generateId(): string {
  return `dingtalk-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
}
...
export function DingTalkTab({ loading }: DingTalkTabProps) {
  const dingtalkStatus = useIntegrationStatus('dingtalk');
  const instances = useIntegrationInstances('dingtalk');
  const activeInstance = useActiveIntegrationInstance('dingtalk');
  const {
    startPlatform,
    stopPlatform,
  ...
  const isConnected = dingtalkStatus?.connected ?? false;
  const connectionState = dingtalkStatus?.connectionState ?? 'disconnected';
  const errorMessage = dingtalkStatus?.error;
  const errorDetail = dingtalkStatus?.errorDetail;
  ...
  const handleDisconnect = async () => {
    try {
      ...
      log.error('Failed to disconnect DingTalk Bot:', error instanceof Error ? error : new Error(String(error)));
    }
  };
    ...
    <div className="space-y-6">
      <div className="p-4 bg-surface rounded-lg border border-border">
        <div className="flex items-center justify-between mb-4">
          <div>
            <div className="text-sm font-medium text-text-primary">钉钉集成</div>
            <div className="text-xs text-text-secondary">通过钉钉机器人接收和发送消息</div>
          </div>
        </div>
                ...
                >
                  ...
                  <div className="text-xs text-text-tertiary mt-1">
                    {instance.config.appId
                      ? `App Key: ${instance.config.appId.slice(0, 8)}...`
                      : '未配置'}
                  </div>
                </div>
              ))}
        ...
        {editingInstance && (
          <div className="p-3 bg-background rounded-lg space-y-4">
            ...
            {/* App Key */}
            <div>
              <label className="block text-xs text-text-secondary mb-2">App Key</label>
              <input
                type="text"
                value={editingInstance.config.appId}
                onChange={(e) => updateEditingConfig({ appId: e.target.value })}
                placeholder="钉钉开放平台应用的 App Key"
                className="w-full px-3 py-2 bg-surface border border-border rounded-lg text-sm text-text-primary focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
                disabled={loading}
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                placeholder="钉钉开放平台应用的 App Secret"
                className="w-full px-3 py-2 bg-surface border border-border rounded-lg text-sm text-text-primary focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
                disabled={loading}
              />
            </div>
            ...
            {/* Webhook URL */}
            <div>
              <label className="block text-xs text-text-secondary mb-2">Webhook URL（用于回复消息）</label>
              <input
                type="text"
                value={editingInstance.config.webhookUrl || ''}
                onChange={(e) => updateEditingConfig({ webhookUrl: e.target.value })}
                placeholder="钉钉群机器人的 Webhook 地址（https://oapi.dingtalk.com/robot/send?access_token=...）"
                className="w-full px-3 py-2 bg-surface border border-border rounded-lg text-sm text-text-primary focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
                disabled={loading}
              />
              <p className="mt-1 text-xs text-text-tertiary">
                钉钉通过 WebSocket 接收消息，通过 Webhook 发送回复。需在群中添加机器人并获取 Webhook URL。
              </p>
            </div>
          ...
          <div className="flex items-start gap-2">
            ...
            <div className="flex-1">
              ...
              <ul className="text-xs text-text-tertiary mt-1 space-y-1 list-disc list-inside">
                <li>在钉钉开放平台创建企业内部应用并获取 App Key 和 App Secret</li>
                <li>启用「机器人」能力，开启 Stream 模式（WebSocket 长连接）</li>
                <li>在钉钉群中添加机器人，获取 Webhook URL（用于回复消息）</li>
                <li>填写配置后点击「保存」保存实例</li>
                <li>点击「连接」会自动保存并连接</li>
                <li>同一时间只能连接一个钉钉机器人实例</li>
              </ul>
            </div>
          </div>
        </div>
