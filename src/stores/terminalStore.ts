 ...
 * 终端状态管理
 *
 * 综合管理：终端会话 + 终端脚本。原分散在 terminalStore / terminalScriptStore 两个文件，合并后统一入口。
 */
...
interface TerminalScriptState {
  ...
  initScriptEventListeners: () => () => void;
  clearError: () => void;
}

export type TerminalStore = TerminalState & TerminalActions & TerminalScriptState;
...
export const useTerminalStore = create<TerminalStore>()((set, get) => ({
  ...
  stopScript: async (scriptId) => {
    ...
    await get().closeSession(runtime.terminalSessionId);
    set((state) => ({
      runtimes: {
        ...state.runtimes,
