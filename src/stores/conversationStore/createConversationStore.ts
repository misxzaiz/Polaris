  ...
  const store = create<ConversationStore>()(
    subscribeWithSelector((set, get) => ({
      ...
      openThinkingBlock: () => {
        ...
        const { currentMessage, streamingUpdateCounter } = get()
        // 若末段已是 thinking block，复用现有块（防重复）
        if (currentMessage?.blocks[currentMessage.blocks.length - 1]?.type === 'thinking') {
          return
        }
        const block = { type: 'thinking' as const, content: '' }
        if (!currentMessage) {
      ...
      appendIncrementalThinking: (content) => {
        // 先 flush 文本缓冲区，确保文本不丢失
        if (_textBuffer) get()._flushTextBuffer()
        ...
        if (lastBlock?.type === 'thinking') {
          ...
          // 孤儿增量 delta（理论上不会发生），降级为创建新 thinking block
          const newBlock = { type: 'thinking' as const, content }
          if (!currentMessage) {
            set({
              currentMessage: createCurrentAssistantMessage([newBlock]),
              streamingUpdateCounter: streamingUpdateCounter + 1,
            })
          } else {
            const newBlocks = [...blocks, newBlock]
            set({
              currentMessage: { ...currentMessage, blocks: newBlocks },
              streamingUpdateCounter: streamingUpdateCounter + 1,
            })
          }
        }
      ...
      closeThinkingBlock: () => {
        ...
        const { currentMessage, streamingUpdateCounter } = get()
        const blocks = currentMessage?.blocks ?? []
        // 若末段不是 thinking block，静默忽略
        if (blocks[blocks.length - 1]?.type !== 'thinking') {
          return
        }
        // 空 thinking block（open 后无 delta）：从 blocks 移除，渲染器不显示空白卡片
        const lastB = blocks[blocks.length - 1] as { type: 'thinking'; content: string } | undefined
        if (lastB && !lastB.content.trim()) {
          const filtered = blocks.filter((_, i) => i !== blocks.length - 1)
          if (filtered.length === 0 && currentMessage) {
            // blocks 全部被移除后，重置 currentMessage（后续文本会重新创建）
            set({
              currentMessage: null,
              streamingUpdateCounter: streamingUpdateCounter + 1,
            })
            ...
            set({
              currentMessage: { ...currentMessage, blocks: filtered },
              streamingUpdateCounter: streamingUpdateCounter + 1,
            })
          }
          return
