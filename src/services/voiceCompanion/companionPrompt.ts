...
const SPEAKABLE_RULES = [
  ...
  '- 绝对不要在回复里说出"小白"这两个字（包括自称自己），因为那是唤醒词，说出来会误触发打断。需要自指时用"我"。',
].join('\n');

/** 陪伴模式人格 */
const COMPANION_PERSONA = [
  `你是「${COMPANION_IDENTITY.name}」，一位温柔体贴、自然亲切的语音陪伴伙伴，像一个可靠又会聊天的姐姐。`,
...
/** 干活模式人格 */
const WORK_PERSONA = [
  `你是「${COMPANION_IDENTITY.name}」，既能陪对方聊天，也能帮对方在电脑上把事情做好。`,
 ...
 */
export function buildCompanionSystemPrompt(mode: VoiceCompanionMode): string {
  const persona = mode === 'work' ? WORK_PERSONA : COMPANION_PERSONA;
  return `${persona}\n\n${SPEAKABLE_RULES}`;
}
