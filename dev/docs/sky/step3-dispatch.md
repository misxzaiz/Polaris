...
### 遗留（阶段 C / 第四步）— 已全部清空
- ✅ 面板阶段 C（已注册能力列表 / dispatch 测试台）— 见 P3
- ✅ 命令层迁移到 dispatch — 见 step4-migration.md（cap.todo 已完整闭环 + 摘旧）
- ✅ 路由 dispatch 直连 SqliteStorage — 见 P2
...
### 第四步（P4）：cap.todo 闭环 + 摘旧 —— 已析出

cap.todo 迁移、摘旧与复盘的完整记录已析出至 **[step4-migration.md](step4-migration.md)**（第四步自此为持续迁移主文档，后续迁移块不再寄生本文件）。
...
完整复盘见 [step4-migration.md](step4-migration.md) §3；可见性修法独立成篇：[plans/plugin-visibility-plan.md](../../plans/plugin-visibility-plan.md)。
