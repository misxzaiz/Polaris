...
| [step7-consolidation.md](step7-consolidation.md) | **第七步：完全重构 · 单一入口收敛** ✅ 阶段 A 已实施（C2 AI 聊天上总线）；阶段 A/B1/B2 已实施（AI 聊天 + cap.context + cap.history）；阶段 E 前置落地（bus MCP server，AI 可经 MCP 读写 todo）；C/D/E 余项待启动 |
| [step8-config.md](step8-config.md) | **第八步：系统配置存储改造** 规划定稿（cap.config 上总线）：config 维持 config.json 单一真源不切库，收敛读写路径 + 权限门 + 审计 + 摘旧三份复刻；第七步阶段 C 第一块 |
